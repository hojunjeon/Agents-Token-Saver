...
export function buildBypassNotice(fullOutputPath) {
...
export function buildCompactionNotice(result, fullOutputPath) {
    const hints = [];
    if (result.rawRef?.id) {
        hints.push(`raw artifact: ${result.rawRef.id}`);
    }
    if (result.rawRef?.path) {
        hints.push(`full output: ${result.rawRef.path}`);
    }
    if (fullOutputPath) {
...
export function buildTokenjuiceDetails(result) {
    ...
    return {
        ...
        ...(result.rawRef?.metadataPath ? { rawArtifactMetadataPath: result.rawRef.metadataPath } : {}),
        ...(typeof result.classification?.matchedReducer === "string"
            ? { reducer: result.classification.matchedReducer }
            : {}),
    };
}
export function mergeDetails(existingDetails, tokenjuiceDetails) {
