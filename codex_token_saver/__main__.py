...
def main(argv: list[str] | None = None) -> int:
    ...
    p_get.add_argument("--db", default=".codex-token-saver/context.sqlite")
    p_get.add_argument("--preview", action="store_true")

    p_search = sub.add_parser("search", help="search captured raw evidence")
    p_search.add_argument("query")
    p_search.add_argument("--db", default=".codex-token-saver/context.sqlite")
    ...
    p_pack.add_argument("--db", default=".codex-token-saver/context.sqlite")
    ...
    p_post.add_argument("--db", default=os.environ.get("CODEX_TOKEN_SAVER_DB", ".codex-token-saver/context.sqlite"))
    p_post.add_argument(
        "--threshold-bytes",
        type=int,
        default=int(os.environ.get("CODEX_TOKEN_SAVER_HOOK_THRESHOLD", str(DEFAULT_THRESHOLD_BYTES))),
    )
    ...
    p_install_hook.add_argument("--db", default=os.environ.get("CODEX_TOKEN_SAVER_DB", ".codex-token-saver/context.sqlite"))
    p_install_hook.add_argument("--cts-command")
    ...
    if args.cmd == "pack":
        ...
        print(pack.text, end="")
        print(f"\n[cts] baseline={pack.baseline_tokens} optimized={pack.optimized_tokens} saving={pack.saving_ratio:.1%} recall={pack.anchor_recall:.0%}")
        return 0
    ...
    if args.cmd == "install-hook":
        shim = install_post_tool_use_hook(Path(args.codex_home), Path(args.db), cts_command=args.cts_command)
        print("Installed Codex Token Saver PostToolUse hook:")
        print(f"  shim: {shim}")
...
def _agents_template() -> str:
    return """# Codex Token Saver

Use `cts pack --query "<task>"` before broad file reads. Use `cts filter --capture --command "<cmd>"` for noisy terminal output. Keep final replies concise, but preserve exact file paths, errors, commands, and decisions. If compacted context is insufficient, retrieve exact raw evidence with `cts get <id>` from the shown `ctx://capture/<id>` reference.
"""
