...
class RequirementWatchdog:
    ...
    def _gate_codex_assets(self) -> Gate:
        skill = self.root / "skill" / "codex-token-saver" / "SKILL.md"
        text = skill.read_text(encoding="utf-8") if skill.exists() else ""
        ok = "Codex" in text and "cts" in text and "AGENTS.md" in text and "requirement-watchdog.md" in text
        return Gate("Codex skill and AGENTS workflow", "PASS" if ok else "FAIL", str(skill))

    def _gate_watchdog_subagent(self) -> Gate:
        path = self.root / "skill" / "codex-token-saver" / "agents" / "requirement-watchdog.md"
        text = path.read_text(encoding="utf-8") if path.exists() else ""
        expected = [
            "cts watchdog --run-tests --until-pass",
            "94%",
            "anchor recall 100%",
        ]
    ...
    def _gate_post_tool_use_hook(self) -> Gate:
        files = [
            self.root / "codex_token_saver" / "hook.py",
            self.root / "tests" / "test_hook_and_install.py",
        ]
    ...
    def _gate_windows_zip(self) -> Gate:
        path = self.root / "dist" / "codex-token-saver-windows.zip"
